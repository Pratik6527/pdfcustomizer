document.addEventListener('DOMContentLoaded', () => {
  ...
  const appState = {
    ...
    downloadUrl: null,
    hasPendingManualEdits: false,   // ← NEW: tracks if manual edits happened
    exportQuality: 'smart'          // ← NEW: sync from selector
  };
  ...
  function setDownloadReady(downloadUrl) {
    ...
    appState.hasPendingManualEdits = false;
    btnDownload.disabled = false;
    btnDownload.classList.remove('btn-disabled', 'btn-needs-export');
    btnDownload.innerHTML = '<span class="btn-icon">⬇</span> Download Clean PDF';
    btnDownload.dataset.downloadUrl = downloadUrl;
  }

  // ===== NEW: Mark PDF as stale after manual edits =====
  function markDownloadStale() {
    appState.hasPendingManualEdits = true;
    appState.downloadUrl = null;
    btnDownload.disabled = false;
    btnDownload.classList.remove('btn-disabled');
    btnDownload.classList.add('btn-needs-export');
    btnDownload.innerHTML = '<span class="btn-icon">📤</span> Export Updated PDF';
  }
  ...
  btnDownload.addEventListener('click', async () => {
    if (!appState.processedReady) {
        showToast('Please process the document first.', 'error');
        return;
    }

    // If manual edits exist, rebuild PDF first
    if (appState.hasPendingManualEdits || !appState.downloadUrl) {
        await exportUpdatedPDF();
        return;
    }

    // Use cached Blob URL
    triggerBlobDownload(appState.downloadUrl);
  });

  // ===== NEW: Export PDF from current preview state =====
  async function exportUpdatedPDF() {
    showLoading('Building updated PDF from your edits...');
    try {
        const res = await fetch('/api/export', {
            ...
            body: JSON.stringify({
                file_id: appState.fileId,
                quality_mode: selExportQuality ? selExportQuality.value : 'smart'
            })
        });
        const data = await safeJsonResponse(res);
        ...
        if (data.pdf_base64) {
            ...
            // Large file: use direct URL
            setDownloadReady(data.download_url);
            window.open(data.download_url, '_blank');
        }

        showToast(`PDF exported! ${data.generated_size_mb}MB — all manual edits included.`, 'success');
    } catch (err) {
  ...
  // ===== NEW: Blob download helper =====
  function triggerBlobDownload(url) {
    const a = document.createElement('a');
    a.href = url;
    a.download = `cleaned_${(appState.fileId || 'document').slice(0, 8)}.pdf`;
  ...
  async function sendBrushStrokeToBackend(pageNumber, points, size) {
    ...
    try {
      ...
      markDownloadStale();    // ← ADD THIS LINE
      showToast('Brush applied. Click "Export Updated PDF" to download.', 'info');
    } catch (err) {
      showToast(err.message, 'error');
  ...
  async function sendRectangleToBackend(pageNumber, rect) {
    ...
    try {
      ...
      refreshGeneratedPages(data.affected_pages || [pageNumber]);
      markDownloadStale();    // ← ADD THIS LINE
      showToast('Area erased. Click "Export Updated PDF" to download.', 'info');
    } catch (err) {
      showToast(err.message, 'error');
      const shell = document.querySelector(`.generated-page[data-page="${pageNumber}"]`);
      if (shell) {
